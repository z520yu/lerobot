# 文本转化为token 处理离散state （pi05比pi0多的）
...
class PaligemmaTokenizer:
    """
    PaliGemma tokenizer用于将文本转化为token,处理离散state输入 在config被调用
    """
...
class FASTTokenizer:
    def __init__(self, max_len: int = 256, fast_tokenizer_path: str = "physical-intelligence/fast"):
        ...
        self._fast_skip_tokens = 128  # 跳过 PaliGemma 词汇表中最后 128 个 token，因为它们是特殊 token
    ...
    def tokenize(
        ...
        # 约定：状态被离散化为 256 个离散区间（归一化后假设范围：[-1, 1]）
        discretized_state = np.digitize(state, bins=np.linspace(-1, 1, 256 + 1)[:-1]) - 1
        ...
        if actions is not None:
            # 使用 FAST tokenizer 对动作进行分词 --> 映射到 PaliGemma 词汇表中的最后 token
            action_tokens = self._fast_tokenizer(actions[None])[0]
            action_tokens_in_pg = self._act_tokens_to_paligemma_tokens(action_tokens)

            # 约定：后缀包含 'Action:' 后跟 FAST token，再后跟 '|'
            postfix_tokens = (
        ...
        # 创建输出 token 序列和掩码
        # AR 掩码在前缀上为 0（双向注意力），在后缀上为 1（对所有先前 token 的因果注意力）
...
###########################################################################
## 下面的 tokenizer 用于 RoboArena 基线实现。                            ##
## 它们*不*用于 pi0 风格的模型。                                          ##
###########################################################################
...
class BinningTokenizer:
    """
    标准 RT-2 / OpenVLA 风格的区间化 tokenizer。
    """
