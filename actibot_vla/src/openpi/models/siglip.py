...
"""为 Pi 重构和简化的 ViT 适配，取自 big_vision。"""
...
class _Module(nn.Module):
    ...
    dropout: float = 0.0
    pool_type: str = "gap"  # 也可以是 "map" 或 "tok"
    head_zeroinit: bool = True
    scan: bool = False
    # 或 "dots_with_no_batch_dims_saveable" 以获得更高速度（内存消耗大）
    remat_policy: str = "nothing_saveable"
    dtype_mm: str = "float32"
    ...
    def __call__(self, image, *, train=False):
        ...
        # Kevin 编辑：在 float32 中进行 patch 提取和 posemb，
        # 因为我觉得这样更安全一些。
        ...
        # Kevin 编辑：现在转换回 dtype_mm（可能是半精度）
        ...
        if self.rep_size:
            ...
            # 注意：过去我们在 pre_logits 中没有包含 tanh。
            # 对于 few-shot，这应该不太重要，因为它无论如何都会白化。
