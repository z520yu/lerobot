...
class PiperEffortROS2Node(Node):
    ...
    def _joint_state_cb(self, msg: JointState) -> None:
        ...
        if msg.position and len(msg.position) >= 7:
            self._queue_joint.put(np.asarray(msg.position[:7], dtype=np.float64))
                ...
                self._queue_joint.put(np.concatenate([base, [0.0]]))
        
        # 处理effort信息
        if self._queue_effort.full():
            self._queue_effort.get_nowait()
        if msg.effort and len(msg.effort) >= 7:
            self._queue_effort.put(np.asarray(msg.effort[:7], dtype=np.float64))
        else:
            # 如果没有effort，填充为0
            self.get_logger().info(f"关节力矩信息缺失")
            self._queue_effort.put(np.zeros(7, dtype=np.float64))
        # 处理velocity信息
        if self._queue_velocity.full():
            self._queue_velocity.get_nowait()
        if msg.velocity and len(msg.velocity) >= 7:
            self._queue_velocity.put(np.asarray(msg.velocity[:7], dtype=np.float64))
        else:
            self.get_logger().info(f"关节速度信息缺失")
            self._queue_velocity.put(np.zeros(7, dtype=np.float64))
...
class PiperROS2Bridge:
    ...
    def __init__(self, config: PiperROS2Config) -> None:
        ...
        self.node: Optional[PiperEffortROS2Node] = None

    def start(self) -> None:
        if not rclpy.ok():
            rclpy.init()
        self.node = PiperEffortROS2Node(self._config)
