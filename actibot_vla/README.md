...
### 克隆代码
```bash
git clone git@gitlab.acti.bot:tech/manipulation/parcel_sorting/pi05.git
```
...
```
注意如果不设置GIT_LFS_SKIP_SMUDGE可能导致部分包因为太大无法下载而报错
### 配置torch环境支持

torch版本 默认是cuda12.6 但是无法支持5090 所以需要torch的cuda12.8及以上
```bash
uv pip uninstall torch
uv pip install torch==2.7.1 torchvision==0.22.1 torchaudio==2.7.1 --index-url https://download.pytorch.org/whl/cu128
```
将transformer的包替换一部份--先验证版本是否是4.53.2
```bash
uv pip show transformers
cp -r ./src/openpi/models_pytorch/transformers_replace/models/* .venv/lib/python3.11/site-packages/transformers/models/
```
...
#### 先转化base model
注意 config name主要用于区分pi0还是pi05，对应上就好。另外本次转化并不转化jax权重中的lora，所以之前如果是jax的lora训练（冻结原始权重训练lora权重）的转化过来几乎相当于没有被训练的base model的能力。
```bash
   python examples/convert_jax_model_to_pytorch.py \
     --checkpoint-dir pretrained_models/openpi-assets/checkpoints/pi05_base \
     --config-name pi05_full_droid_finetune \
     --output-path pretrained_models/torch/pi05_base
```

### 编写[config](src/openpi/training/config.py)的内容
...
```bash
  ...
  python scripts/train_pytorch.py pi05_single_piper_torch --exp_name pytorch_ddp_test --resume  # Resume from latest checkpoint
```
多个GPU（单节点）:
```bash
  torchrun --standalone --nnodes=1 --nproc_per_node=<num_gpus> scripts/train_pytorch.py <config_name> --exp_name <run_name>
  Example:
  torchrun --standalone --nnodes=1 --nproc_per_node=2 scripts/train_pytorch.py pi05_single_piper_torch --exp_name pytorch_ddp_test
  torchrun --standalone --nnodes=1 --nproc_per_node=2 scripts/train_pytorch.py pi05_single_piper_torch --exp_name pytorch_ddp_test --resume
```
多节点:
```bash
	torchrun \
...
tmux attach -t train_name

tmux kill-session -t train_name
```
